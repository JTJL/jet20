...
def solve(p,x,config,fast,should_stops=None):
    if not p.obj:
        raise Exception("no obj found")

    if p.eq is not None and p.le is not None:
        return primal_dual_interior_point_with_eq_le(x,p.obj,p.le,p.eq,should_stops,fast=fast,**config.get_namespace("opt"))

    elif p.eq is not None:
        return primal_dual_interior_point_with_eq(x,p.obj,p.eq,should_stops,fast=fast,**config.get_namespace("opt"))

    elif p.le is not None:
        return primal_dual_interior_point_with_le(x,p.obj,p.le,should_stops,fast=fast,**config.get_namespace("opt"))
